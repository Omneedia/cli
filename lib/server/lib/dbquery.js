  ...
  exec: function (o, cb) {
    ...
    async function querycommander(o, PARAMS, cb) {
      ...
      async function query_fields(q) {
        ...
        if (qvalue.indexOf("{") == -1) {
          if (qvalue.indexOf("=") > -1)
            qvalue = qvalue.substr(qvalue.indexOf("=") + 1, qvalue.length);
        }
          ...
          if (qvalue.indexOf("[") > -1) {
            ...
            if (qvalue.indexOf("{") > -1) {
              ...
              var zo = [
                ...
                qvalue.substr(
                  qvalue.indexOf("[") + 1,
                  qvalue.lastIndexOf("]") - qvalue.indexOf("[") - 1
                ) + ":sql",
              ];
            ...
            if (operator == "!=") {
              return (
                qkey + " not in (" + qvalue.split("[")[1].split("]")[0] + ")"
              );
            } else {
            ...
            // cas d'une fonction
            if (q.indexOf("(") > -1) return qkey + "=" + qvalue + "";
            else {
              if (qvalue.indexOf("{") > -1) {
                // in another table
                var zo = [
                  o[0],
                  qvalue.substr(qvalue.indexOf("=") + 1, qvalue.length) +
                    ":sql",
                ];
                var result = await qcommander(zo, {});
                qvalue = "(" + result + ")";
      ...
      for (var i = 0; i < MSettings.db.length; i++) {
        if (MSettings.db[i].name == o[0])
          _db = MSettings.db[i].uri
            .substr(
              MSettings.db[i].uri.lastIndexOf("/") + 1,
              MSettings.db[i].uri.length
            )
            .split("?")[0];
      }
      ...
      /*
            ...
            */
      if (cmd.indexOf("@") == 0) {
        db.model(o[0], "SHOW COLUMNS FROM " + cmd.split("@")[1], function (
          e,
          r
        ) {
          cb(e, r);
      ...
      var sql =
        "select CONSTRAINT_NAME, TABLE_NAME,COLUMN_NAME, REFERENCED_TABLE_NAME,REFERENCED_COLUMN_NAME from INFORMATION_SCHEMA.KEY_COLUMN_USAGE where CONSTRAINT_SCHEMA='" +
        _db +
        "' order by TABLE_NAME";
      ...
      var primary = {};
      for (var i = 0; i < r.length; i++) {
        if (r[i].CONSTRAINT_NAME == "PRIMARY")
          primary[r[i].TABLE_NAME] = r[i].TABLE_NAME + "." + r[i].COLUMN_NAME;
        if (r[i].REFERENCED_TABLE_NAME && table != r[i].REFERENCED_TABLE_NAME) {
          if (!joins[r[i].REFERENCED_TABLE_NAME])
            joins[r[i].REFERENCED_TABLE_NAME] =
              "LEFT JOIN " +
              r[i].REFERENCED_TABLE_NAME +
              " ON " +
              r[i].TABLE_NAME +
              "." +
              r[i].COLUMN_NAME +
              ...
              r[i].REFERENCED_TABLE_NAME +
              "." +
              r[i].REFERENCED_COLUMN_NAME;
          else {
            if (Array.isArray(joins[r[i].REFERENCED_TABLE_NAME])) {
              joins[r[i].REFERENCED_TABLE_NAME].push(
                "LEFT JOIN " +
                  r[i].REFERENCED_TABLE_NAME +
                  " ON " +
                  r[i].TABLE_NAME +
      ...
      if (ORDERBY.length > 0) {
        ...
        for (var i = 0; i < ORDERBY.length; i++) {
          if (ORDERBY[i].indexOf("-") > -1)
            order_by.push(ORDERBY[i].split("-")[0].split("=")[0] + " DESC");
    ...
    if (!o.__SQL__) {
      ...
      if (o.__SQL__.indexOf("!") > -1) {
        o.__SQL__ = CYPHER_decode(
          "0mneediaRulez!",
          o.__SQL__.substr(1, o.__SQL__.length)
        );
      }
      ...
      if (o.__SQL__.indexOf("?") > -1) {
        var tt = o.__SQL__
          .substr(o.__SQL__.indexOf("?") + 1, o.__SQL__.length)
          .split("&");
        var cc = {};
        var listargs = [];
