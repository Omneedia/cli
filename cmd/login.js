module.exports = function (args, root) {
  ...
  logger(root,function(x) {
    if (x) return console.log('\n'+logSymbols.success+" Already logged in ("+chalk.bold(x.info.username)+")\n");
    ...
    inquirer.prompt(questions).then(function (answers) {
      Request(
        ...
        function (err, resp, body) {
          ...
          fs.writeFile(root + "/.login", response.token,function (e, s) {
            console.log('\n\t'+logSymbols.success+' '+chalk.green.bold('You are logged in.\n'));
          });
        }
      );
    });
