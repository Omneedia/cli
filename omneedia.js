 ...
 * Copyright (c) 2017-2020 OmneediaFramework
 */
...
function parseArgs(c) {
    ...
    if (cmdo[cmd].subcommands) {
      ...
      for (var i = 0; i < cmdo[cmd].subcommands.length; i++) {
        if (
          ...
          try {
            return require("./cmd/" + cmdo[cmd].cmd)(args, rootOS);
          } catch (e) {
            console.log(e);
  ...
  for (var el in c) {
    ...
    for (var i = 0; i < item.length; i++) {
      ...
      if (item[i].alias) {
        cmds.push(item[i].alias);
        cmdo[item[i].alias] = item[i];
      }
    }
  }

  for (var i = 0; i < cmds.length; i++) {
    if (process.argv[2] == cmds[i]) return parseCommand(cmds[i]);
  }
        ...
        if (obj.comments) {
          ...
          if (obj.display) {
            ...
            if (Array.isArray(obj.description)) {
              ...
              var str = txt + obj.description;
              if (obj.alias)
                str += chalk.grey(" - alias: ") + chalk.cyan(obj.alias);
              var words = str.length;
              var ndx = 0;
              while (ndx <= words) {
                if (ndx == 0)
...
fs.readFile(__dirname + "/package.json", "utf-8", function (e, ab) {
  ...
  fs.mkdir(rootOS, { recursive: true }, function () {
    return fs.readFile(rootOS + "/.login", "utf-8", function (e, s) {
      if (s) global.Auth = require("jsonwebtoken").decode(s).info;
      else global.Auth = {};
      global.token = s;
        ...
        if (e) {
          ...
          fs.writeFile(
            ...
            function () {
              ...
              if (process.env.proxy) global.config.proxy = process.env.proxy;
          ...
          findUp("manifest.yaml").then(function (test) {
            if (test) global.dir = require("path").dirname(test);
            fs.readFile(global.dir + "/manifest.yaml", "utf-8", function (
              e,
              r
            ) {
              if (r) global.manifest = require("yaml").parse(r);
              parseArgs(commands);
            });
          });
        }
